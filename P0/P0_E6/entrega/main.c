...
int main(int argc, char** argv) {
    // int i, k, f, c; as variables i e k non se empregan
    int f, c;
    float valor;
    ...
    do {
        ...
        switch (opcion[0]) {
            case '1':
                // Liberamos a memoria das matrices antes de crealas no caso de existir
                destrueMatriz(a);
                destrueMatriz(b);
                
                printf("Introduza o numero de filas da matriz 'a': ");
                //scanf("%f", &f); Intentase gardar o valor dun float nun int
                scanf("%d", &f);
            ...
            case '2':
                ...
                for (f = 0; f < b->filas; f++)
                    for (c = 0; c < b->columnas; c++) {
                        printf("Introduza o valor da posicion %dx%d: ", f + 1, c + 1);
                        scanf("%f", &valor);
                        //asignarElemento(f, c, valor, a); Estase dando 2 veces os valores a matriz a
            ...
            case '3':
                do {
                    ...
                    fflush(stdin);
                    //scanf("%s", &resp); Da un aviso ao compilar
                    scanf("%s", resp);
                } while (resp[0] != 'a' && resp[0] != 'b');
                printf("Introducir fila e columna (fila columna): ");
                scanf("%d %d", &f, &c);            
                if (resp[0] == 'a') {
                    //valor = obterElemento(f, c, b); Estase obtendo elemento de b cando se da o caso de a
                    valor = obterElemento(f-1, c-1, a);
                    printf("O valor da fila e columna seleccioada e: %f\n", valor);
                } else if (resp[0] == 'b') {
                    //valor = obterElemento(f, c, a); Viceversa que o caso anterior
                    valor = obterElemento(f-1, c-1, b);
                    printf("O valor da fila e columna seleccioada e: %f\n", valor);
                }
                break;
            case '4':
    ...
    // Destruimos as matrices no caso de que se crearan e non se saira correctamente
    destrueMatriz(a);
    destrueMatriz(b);
